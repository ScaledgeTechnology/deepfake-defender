...
def find_compatible_python():
    ...
    for exe, v in versions:
        print(f"  ✅Your system having python version - {'.'.join(map(str, v))}")
        print(f"  📍On the location - {exe}")
...
def main():
    ...
    if not python_exec:
        print("❌ No compatible Python version found (must be between 3.10 and 3.12.6).")
        print("🔗 Please install a supported version(from the official site):👉 https://www.python.org/downloads/")
        # sys.exit(1)
         # Wait for key press before closing
        print("\nPress Enter key to terminate...")
        input()
        return  # Instead of sys.exit(1), we just return to avoid abrupt termination
...
# To create .exe file (delete build folder, .spec file and .exe file)
# Then run-> pyinstaller --onefile django_app.py
# Then cut .exe file from dist folder to root folder